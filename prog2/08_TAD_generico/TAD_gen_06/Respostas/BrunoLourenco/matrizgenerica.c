...
{
    tMatrizGenerica *matriz = malloc(sizeof(tMatrizGenerica));

    matriz->mtx = malloc(sizeof(void**)*linhas);

    int i;
    for(i = 0; i < linhas; i++)
    {
        matriz->mtx[i] = malloc(sizeof(void*)*colunas);
    }
    ...
    for(i = 0; i < linhas; i++)
    {
        for(j = 0; j < colunas; j++)
        {
            matriz->mtx[i][j] = malloc(numByteElem);
        }
    }

    matriz->linhas = linhas;
    matriz->colunas = colunas;
    matriz->tamByte = numByteElem;
...
{
    memcpy(ObtemElementoMatrizGenerica(mat, linha, coluna), elem, mat->tamByte);
}
