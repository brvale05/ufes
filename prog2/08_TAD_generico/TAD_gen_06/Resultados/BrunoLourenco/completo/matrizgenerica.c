...
{
    tMatrizGenerica *matriz = malloc(sizeof(struct matrizgenerica));

    if(!matriz)
    {
        printf("erro ao alocar tmatriz!\n");
        exit(0);
    }

    matriz->mtx = malloc(linhas*sizeof((void*)numByteElem));

    if(!matriz->mtx)
    {
        printf("erro ao alocar linhas tmatriz!\n");
        exit(0);
    }
    ...
    for(i = 0; i < linhas; i++)
    {
        matriz->mtx[i] = malloc(colunas*sizeof((void)numByteElem));

        if(!matriz->mtx[i])
        {
            printf("erro ao alocar colunas tmatriz!\n");
            exit(0);
        }
    }
